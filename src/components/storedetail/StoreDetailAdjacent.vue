<template>
  ...
  <div v-else-if="adjacentStores.length > 0 && !isLoading" >
    <div class="ml-5 mt-5">
      <span class="header5-medium color-gray800">근처 팝업 추천</span>
    </div>
    <div class="d-flex flex-row store-cards">
      <StoreCard
          v-for="(item, i) in adjacentStores"
          :key="i"
          :store="item"
      />
    </div>
...
const adjacentStores = storeDetailStore.adjacentStoresData as AdjacentStoreResponse[];
